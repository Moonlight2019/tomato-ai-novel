...
请用单句公式概括故事本质。参考句式（按内容指导的基调二选一，仅示意，不必照抄）：
- 常规/高张力向："当[主角]遭遇[核心事件]，必须[关键行动]，否则[灾难后果]；与此同时，[更大的变数]正在酝酿。"
- 轻松向："当[主角]卷入[有趣的事件]，靠着[金手指/优势]一次次[化解麻烦 / 花式打脸]；在[热闹有趣的日常]里，[惊喜反转]悄悄浮现。"

要求：
1. 冲突与情绪强度必须贴合内容指导中的基调——若内容指导要求"轻松/愉快/搞笑/治愈/不涉及阴谋"，则不得出现幕后阴谋、隐藏黑手、灭世危机、亲友背叛等设定
2. 体现人物核心驱动力
3. 可埋下让读者好奇的"悬念疑问"，但悬念形式贴合基调（轻松向=惊喜/误会/有趣反转）
4. 使用25-100字精准表达
...
- 与至少两个其他角色的价值观冲突
- 一个合作纽带
- 一个潜在反转（轻松基调＝误解/玩笑性质的"背叛感"，最终归于和解，而非真正的叛变）

基调贴合（按内容指导执行）：
- 若内容指导要求"轻松/愉快/搞笑/治愈"，角色缺陷与弱点用可笑的倒霉处境、小毛病为主（社死、穷、嘴硬、怕事），"打脸对象"用势利眼同事、爱显摆的人，禁止黑化与阴谋家设定
- 若内容指导要求"黑暗/虐/权谋/惊悚"，才使用狠"惨点"与真正的对手
...
2. 社会维度：
- 权力结构（可引发冲突的阶层/种族/组织摩擦）
- 文化氛围（可被打破的禁忌及其后果）
- 经济命脉（资源争夺焦点）

基调贴合（按内容指导执行）：
- 若内容指导要求"轻松/愉快/搞笑/治愈"，把"权力结构/文化禁忌/经济命脉"写成规矩、面子文化、圈层尴尬、资源摩擦等日常层面，不写成阴谋集团与生死博弈
- 若内容指导要求"黑暗/虐/权谋/惊悚"，才写阶层压迫、秘密组织、资本暗战等
...
【基调控制】以内容指导的基调为最高准则：
- 若内容指导要求"轻松/愉快/搞笑/治愈/不涉及阴谋" → 冲突用生活化小麻烦、误会、幽默打脸；禁止幕后阴谋、隐藏黑手、灭世危机、亲友背叛等设定；"反转/伏笔"用惊喜、误会、有趣反转表达。
- 若内容指导明确要求"黑暗/虐/权谋/惊悚" → 才使用高强度冲突。
- 未明确基调时，只使用题材本身固有的对抗关系，不要凭空升级成"惊天阴谋"。

要求按以下结构设计：
第一幕（触发）
- 日常状态中的异常征兆或有趣由头（3处铺垫）
- 引出故事：展示主线、潜线、副线的开端
- 关键事件：打破平衡的催化剂（需改变至少3个角色的关系；轻松基调可用突发误会/意外/搞笑事件）
- 错误抉择：主角的认知局限导致的错误反应（轻松基调＝弄巧成拙的喜剧爆点）

第二幕（对抗）
- 剧情升级：主线+潜线的交叉点
- 双重压力：外部障碍升级+内部挫折（轻松基调＝麻烦升级+笑料翻倍，无伤亡无阴谋）
- 虚假胜利：看似解决实则引出新麻烦的转折（轻松基调＝以为稳了却当场翻车）
- 灵魂黑夜：认知被颠覆的时刻（轻松基调＝发现自己一直搞错关键真相，哭笑不得而非绝望）

第三幕（解决）
- 代价显现：解决问题的必要代价（轻松基调＝小牺牲或搞笑代价）
- 嵌套转折：至少包含两层反转（三层以上认知颠覆仅限高张力基调）
- 余波：留下2个开放式悬念因子
...
2. 每章需明确：
- 章节定位（角色/事件/主题等）
- 核心悬念类型（信息差/道德困境/时间压力等，贴合内容指导基调）
- 情感基调迁移（如从疑惑→恍然→决心；轻松基调用"哭笑不得→卯足劲→得意"）
- 伏笔操作（埋设/强化/回收）
- 认知颠覆强度（1-5级）

钩子基调要求：章末钩子与情感强度贴合内容指导的基调——若指导要求"轻松/愉快/不涉及阴谋"，优先用突然揭示/未完成动作/神秘物品/意象钩子等非威胁型钩子，禁止用性命威胁或灭世危机；若指导明确要求"黑暗/虐/权谋/惊悚"则不受限。
