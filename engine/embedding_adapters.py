...
class QwenEmbeddingAdapter(BaseEmbeddingAdapter):
    """
    通义千问 Embedding 适配器（直接用 OpenAI SDK，绕过 langchain 兼容问题）
    """
    def __init__(self, api_key: str, base_url: str, model_name: str):
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        try:
            ...
            logging.error(f"Qwen embed_documents error: {e}")
    ...
    def embed_query(self, query: str) -> List[float]:
        try:
            ...
            logging.error(f"Qwen embed_query error: {e}")
            return []


class MimoEmbeddingAdapter(BaseEmbeddingAdapter):
    """
    用 mimo-v2.5 提取关键词，再哈希为定长向量。
    不依赖真实 embedding API，适合无 embedding 服务时的降级方案。
    原理：LLM 提取关键词 → 关键词哈希到固定维度 → 向量
    """
    def __init__(self, api_key: str, base_url: str, model_name: str, dim: int = 256):
        import anthropic
        self.dim = dim
    ...
    def _keywords_to_vector(self, keywords: str) -> List[float]:
        """将关键词文本哈希为固定维度的向量"""
        import hashlib
        vec = [0.0] * self.dim
        # 按逗号/空格分词
        words = [w.strip() for w in keywords.replace('，', ',').replace('、', ',').split(',') if w.strip()]
        if not words:
            words = [keywords]
        for i, word in enumerate(words):
            h = hashlib.md5(word.encode('utf-8')).digest()
            for j in range(min(16, self.dim)):
                idx = (h[j] + i * 17) % self.dim
                vec[idx] += 1.0
        # 归一化
    ...
    def _extract_keywords(self, text: str) -> str:
        ...
        try:
            ...
            resp = self._client.messages.create(
                ...
                messages=[{
                    "role": "user",
                    "content": f"从以下文本中提取5-8个核心关键词，仅用逗号分隔输出，不要解释：\n{text}"
                }],
            )
            ...
            logging.error(f"MimoEmbedding keyword extraction failed: {e}")
            return text[:50]
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for text in texts:
            ...
            results.append(self._keywords_to_vector(kw))
        return results

    def embed_query(self, query: str) -> List[float]:
        kw = self._extract_keywords(query)
        return self._keywords_to_vector(kw)
...
def create_embedding_adapter(
    ...
    if fmt == "openai":
        ...
        return QwenEmbeddingAdapter(api_key, base_url, model_name)
    elif fmt == "mimo":
        return MimoEmbeddingAdapter(api_key, base_url, model_name)
    else:
