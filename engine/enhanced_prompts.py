...
# =============== 基调控制 ===============
# 全局基调/黑暗度控制，注入到架构、角色、世界、大纲等 prompt，优先级最高。
# 触发条件：用户"内容指导"中的基调关键词 + 题材默认基调。
LIGHT_TONE_GENRE_KEYWORDS = ("甜宠", "种田", "日常", "青春", "轻松", "治愈", "爆笑", "沙雕", "萌", "甜")

DEFAULT_TONE_RULE_STANDARD = (
    "按本题材的常规张力编写；冲突只来自题材本身固有的对抗关系，"
    "不要凭空加入“幕后黑手 / 惊世阴谋 / 灭世危机 / 亲友背叛”等设定。"
)
DEFAULT_TONE_RULE_LIGHT = (
    "本题材默认偏轻松向：全篇以幽默、治愈、愉快的冲突为主，禁止阴谋、黑暗、死亡、背叛、末日等设定；"
    "反转与伏笔改用惊喜、误会、妙趣揭晓等轻松方式表达。"
)

TONE_CONTROL = """【基调控制 · 最高优先级，先于一切结构要求执行】
1. 当“内容指导”（用户提供）包含“轻松 / 愉快 / 搞笑 / 欢乐 / 温暖 / 治愈 / 可爱 / 甜 / 开心 / 不要虐 / 不涉及阴谋”等表达 → 基调＝轻松：
   - 允许的冲突：生活化小麻烦、误会、幽默打脸、搞笑困境、成长小挫折；
   - 禁止：幕后阴谋集团、隐藏黑手、惊世阴谋、灭世危机、亲友背叛、不可逆的死亡与黑暗反转；
   - “悬念 / 伏笔 / 反转”改为：惊喜揭晓、身份误会、妙趣反转、温馨波折。
2. 当“内容指导”包含“黑暗 / 虐 / 深沉 / 权谋 / 惊悚 / 恐怖 / 惨烈”等表达 → 基调＝高张力：可使用阴谋、背叛、生死危机等元素。
3. 未明确基调时 → {tone_rule}"""


def patch_prompt_definitions(genre: str = "都市") -> None:
    """把指定题材的增强版 prompt 覆盖到 prompt_definitions 全局模块，供流水线 CLI 与 GUI 共用。"""
    import prompt_definitions

    enhanced = get_enhanced_prompts(genre)
...
请用单句公式概括故事本质。参考句式（按基调二选一，仅示意，不必照抄）：
- 常规/高张力向："当[主角]遭遇[核心事件]，必须[关键行动]，否则[灾难后果]；与此同时，[更大的变数]正在酝酿。"
- 轻松向："当[主角]卷入[有趣的事件]，靠着[金手指/优势]一次次[化解麻烦 / 花式打脸]；在[热闹有趣的日常]里，[惊喜反转]悄悄浮现。"

要求：
1. 冲突与情绪强度必须贴合【基调控制】；轻松基调下不得出现幕后阴谋、隐藏黑手、灭世危机、亲友背叛等设定
2. 体现人物核心驱动力（可结合题材公式中的驱动词，如都市=身份落差×信息差×资源博弈，系统流=任务×奖励×升级）
3. 可埋下让读者好奇的"悬念疑问"，但悬念形式必须贴合基调（轻松向=惊喜/误会/有趣反转）
4. 使用25-100字精准表达
5. 轻松基调下：前3章完成"被小看 → 第一次亮眼表现"的循环，情绪以爽+有趣为主；高张力基调下可选"惨 → 翻盘"
...
特征：
- 背景、外貌、性别、年龄、职业等
- 暗藏的秘密或潜在弱点(可与世界观或其他角色有关；轻松基调下用有趣的怪癖/小秘密，不用黑化设定)
...
- 与至少两个其他角色的价值观冲突
- 一个合作纽带
- 一个潜在反转（轻松基调＝可能有误解、玩笑性质的"背叛感"，最终归于和解，而非真正的叛变）

角色基调要求（按【基调控制】执行）：
- 轻松基调：主角的"惨点/缺陷"用可笑的倒霉处境（社死、被坑、欠债、嘴硬、怕事）表达；"打脸对象"用势利眼同事、爱显摆的人、看人不带正眼的路人；禁止把角色写成阴谋家或黑化反派
- 高张力基调：才使用失业/离婚/被欺压等狠"惨点"与真正的对手
...
社会维度的取舍按【基调控制】执行：
- 轻松基调：把"权力结构"写成规矩/面子文化/圈层尴尬，"文化禁忌"写成潜规则与搞笑后果，"经济命脉"写成资源摩擦，不写成阴谋集团与生死博弈
- 高张力基调：才写阶层压迫、秘密组织、资本暗战等
...
要求按以下结构设计（各环节的强度与黑暗程度一律服从【基调控制】）：

第一幕（触发）— 开篇阶段
- 日常状态中的异常征兆或有趣由头（3处铺垫，强度贴合基调）
- 引出故事：展示主线、潜线、副线的开端
- 关键事件：打破平衡的催化剂（需改变至少2-3个角色的关系）
  · 轻松基调：突发误会 / 意外中奖 / 系统解锁 / 当众出糗等，禁止用危机绑架推进
  · 高张力基调：真正的威胁事件
- 错误抉择：角色因性格局限做出的错误反应
  · 轻松基调：莽撞/作死/弄巧成拙的喜剧爆点

第二幕（对抗）— 中间阶段
- 剧情升级：主线+潜线的交叉点
- 双重压力：外部障碍升级 + 内部问题
  · 轻松基调：麻烦升级 + 笑料翻倍，控制在"无伤亡、无阴谋"范围内
  · 高张力基调：危机升级、对手施压
- 虚假胜利：看似解决却引出新麻烦的转折
  · 轻松基调：以为稳了/赢了，结果当场翻车
- 灵魂黑夜：认知被颠覆的时刻
  · 轻松基调：发现自己一直搞错了关键真相，情绪偏惊喜/哭笑不得，而非绝望
  · 高张力基调：世界观/信仰的深层动摇
...
- 单元之间设置"认知过山车"（连续2章紧张→1章缓冲；轻松基调＝连续2章热闹→1章平缓）
- 关键转折章需预留多视角铺垫
- 前3章完成第一次"爽点循环"，之后每5章一个小高潮

2. 每章需明确：
- 章节定位（角色/事件/主题等）
- 核心悬念类型（信息差/道德困境/时间压力等，贴合基调）
- 情感基调迁移（如从疑惑→恍然→决心；轻松基调用"哭笑不得→卯足劲→得意"）
- 伏笔操作（埋设/强化/回收）
- 认知颠覆强度（1-5级，强度贴合基调）
- 【必选】章末钩子类型（从13式中选择，标注具体钩子；轻松基调优先选突然揭示/未完成动作/神秘物品/意象钩子/留白钩子等非威胁型，禁止用性命威胁或灭世危机制造紧张）
...
【基调贴合】
- 整体情绪遵循下方"额外指导"设置的基调；若指导要求"轻松愉快/不涉及阴谋"，本章以幽默轻快、节奏明快为主，禁止阴谋、黑暗、死亡等设定
- 前200字的"冲突或异常"在轻松基调下可以是离谱的遭遇、社死名场面或荒诞巧合
...
🎯 写作核心要求：
1. 与前文摘要、前章结尾段衔接流畅
2. 章末必须使用钩子（突然揭示/未完成动作/身份反转/两难抉择等；若指导要求轻松基调，优先用搞笑反转/惊喜揭晓类钩子，禁止用性命威胁或灭世危机）
3. 段落以1-3句为主，避免均匀节奏
...
9. 整体情绪与冲突强度贴合"用户指导"中设定的基调：若指导要求轻松愉快，本章不得引入阴谋、黑暗、死亡等设定
...
def get_enhanced_prompts(genre: str = "都市") -> dict:
    ...
    # 基调控制：按题材默认基调挑选"未明确要求"时的规则
    if any(kw in genre for kw in LIGHT_TONE_GENRE_KEYWORDS):
        tone_rule = DEFAULT_TONE_RULE_LIGHT
    else:
        tone_rule = DEFAULT_TONE_RULE_STANDARD
    tone_block = TONE_CONTROL.format(tone_rule=tone_rule)
